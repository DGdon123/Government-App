...
class _NinethRouteState extends State<NinethRoute> {
  ...
  Future<List<PanjikaranElement>> data() async {
    final response = await http.get(
      Uri.parse(
        'http://govapp.prabidhienterprises.com/api/nagrikbadapatra/panjikaran',
      ),
    );
    ...
    if (response.statusCode == 200) {
      for (var index in data.values) {
        samplePosts.addAll(List<PanjikaranElement>.from(
            index.map((x) => PanjikaranElement.fromJson(x))));
      }
