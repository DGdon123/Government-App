...
class _EighthScreenState extends State<EighthScreen> {
  ...
  var isLoading = true;
  List<GharjaggaElement> samplePosts = [];
  ...
  Future<List<GharjaggaElement>> data() async {
    final response = await http.get(
      Uri.parse(
        'http://govapp.prabidhienterprises.com/api/nagrikbadapatra/gharjagga',
      ),
    );
    ...
    if (response.statusCode == 200) {
      for (var index in data.values) {
        samplePosts.addAll(List<GharjaggaElement>.from(
            index.map((x) => GharjaggaElement.fromJson(x))));
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('घर | जग्गा | बाटो',
            style: TextStyle(
              color: Color(0xFFFFFFFF),
