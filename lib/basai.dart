...
class EleventhRoute extends StatefulWidget {
  ...
  @override
  State<EleventhRoute> createState() => _EleventhRouteState();
}

class _EleventhRouteState extends State<EleventhRoute> {
  FlutterSecureStorage storage = FlutterSecureStorage();
  var isLoading = true;
  List<Basaishari> samplePosts = [];
  ...
  Future<List<Basaishari>> data() async {
    final response = await http.get(
      Uri.parse(
        'http://govapp.prabidhienterprises.com/api/nagrikbadapatra/basaishari',
      ),
    );
    ...
    if (response.statusCode == 200) {
      for (var index in data.values) {
        samplePosts.addAll(
            List<Basaishari>.from(index.map((x) => Basaishari.fromJson(x))));
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('बसोबास | बसाईसराई',
            style: TextStyle(
              color: Color(0xFFFFFFFF),
